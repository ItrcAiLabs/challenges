...
(function () {
    ...
    ChallengeInviteCtrl.$inject = ['utilities', '$state', '$rootScope', '$stateParams'];
...
(function() {
    ...
    function AuthCtrl(utilities, $state, $rootScope) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        if (response.status == 201) {
                            ...
                            $rootScope.notify("success", "با موفقیت ثبت شد. لطفا آدرس ایمیل خود را تایید کنید!");
                            vm.startLoader("شما را به مسابقات پارسی آرما می برد!");
                            // call utility service
                            var loginParameters = {};
        ...
        vm.verifyEmail = function() {
            ...
            parameters.callback = {
                onSuccess: function() {
                    vm.email_verify_msg = "ایمیل شما با موفقیت تایید شد";
                    vm.stopLoader();
                },
                onError: function() {
                    vm.email_verify_msg = "مشکلی پیش آمد!! لطفا دوباره تلاش کنید.";
                    vm.stopLoader();
                }
            };
        ...
        vm.resetPassword = function(resetPassFormValid) {
            if (resetPassFormValid) {
                ...
                parameters.callback = {
                    ...
                    onError: function() {
                        vm.isFormError = true;
                        vm.FormError = "مشکلی پیش آمد. لطفا دوباره تلاش کنید";
                        vm.stopLoader();
                    }
                };
                utilities.sendRequest(parameters, "no-header");
                ...
                parameters.callback = {
                    ...
                    onError: function(response) {
                        ...
                        try {
                            ...
                            vm.FormError = "مشکلی پیش آمد!لطفا صفحه را مجددا بارگذاری کرده و دوباره تلاش کنید.";
                        }
                        vm.stopLoader();
                    }
                };
...
(function() {
    ...
    function ChallengeCtrl(utilities, loaderService, $scope, $state, $http, $stateParams, $rootScope, $interval, $mdDialog, moment, $location, $anchorScroll, $timeout) {
        ...
        vm.getAllEntriesTestOption = "ارسال های خصوصی";
        vm.showPrivateIds = [];
        ...
        vm.manageWorker = function(action){
            ...
            parameters.callback = {
                ...
                onError: function(response) {
                    ...
                    if (error == undefined){
                        $rootScope.notify("error", " خطایی وجود دارد.");
                    }
                    else {
                        $rootScope.notify("error", " خطایی وجود دارد: " + error);
                    }
                }
            };
            utilities.sendRequest(parameters);
                                    ...
                                    onSuccess: function(response) {
                                        ...
                                        if (status == 200) {
                                            ...
                                            if (vm.existTeam.count === 0) {
                                                ...
                                                vm.paginationMsg = "هیچ گروهی در حال حاضر وجود ندارد. با ایجاد یک گروه جدید شروع کنید!";
                                            } else {
        ...
        vm.toggleParticipation = function (ev, isRegistrationOpen) {
            ...
            if (isRegistrationOpen) {
                ...
                participationModalText = 'مشارکت در مسابقه را میبندید؟';
            } else {
                participationState = 'opened';
                participationModalText = 'مشارکت در مسابقه را باز میکنید؟';
            }
            ...
            $mdDialog.show(confirm).then(function () {
                ...
                parameters.callback = {
                    onSuccess: function() {
                        ...
                        $rootScope.notify('success', 'مشارکت با موفقیت  ' + participationState + ' شد');
                    },
                    onError: function(response) {
                        var details = response.data;
                        $rootScope.notify('error', details.error);
        ...
        vm.makeSubmission = function() {
            if (vm.isParticipated) {
                ...
                if ((fileVal === null || fileVal === "") && (vm.fileUrl === null || vm.fileUrl === "")) {
                    vm.subErrors.msg = "لطفا فایل را بارگذاری کنید یا لینک ارسال را وارد کنید!";
                } else {
                    if (vm.isCurrentSubmissionMetaAttributeValid() !== true) {
                        vm.subErrors.msg = "لطفا ورودی برای ویژگی های متا ارائه دهید!";
                        return false;
                    }
                    vm.isExistLoader = true;
                    vm.loaderTitle = '';
                    ...
                    if (vm.isSubmissionUsingUrl) {
                        ...
                        if (isUrlValid && validExtensions.includes(extension)) {
                            ...
                            vm.stopLoader();
                            vm.subErrors.msg = "لطفاً یک لینک معتبر وارد کنید که به پسوند فایل " + validExtensions + "ختم می شود";
                            return false;
                        }
                    } else {
                    ...
                    parameters.callback = {
                        onSuccess: function() {
                            ...
                            vm.publicationUrl = "";
                            vm.isPublicSubmission = null;
                            $rootScope.notify("success", "ارسال شما با موفقیت ثبت شد");
                            vm.disableSubmit = true;
                        ...
                        onError: function(response) {
                            ...
                            vm.isPublicSubmission = null;
                            if (status == 404) {
                                vm.subErrors.msg = "لطفا مرحله را انتخاب کنید";
                            } else {
                                if (error.error){
                    ...
                    for (var i=0; i<vm.leaderboard.length; i++) {
                        ...
                        if (duration._data.years != 0) {
                            ...
                            if (hours.toFixed(0)==1) {
                                vm.leaderboard[i].timeSpan = 'ساعت';
                            } else {
                                vm.leaderboard[i].timeSpan = 'ساعت';
                            }
                        }
                            ...
                            if (minutes.toFixed(0)==1) {
                                vm.leaderboard[i].timeSpan = 'دقیقه';
                            } else {
                                vm.leaderboard[i].timeSpan = 'دقیقه';
                            }
                        }
                            ...
                            if (second.toFixed(0)==1) {
                                vm.leaderboard[i].timeSpan = 'ثانیه';
                            } else {
                                vm.leaderboard[i].timeSpan = 'ثانیه';
                            }
                        }
            ...
            parameters.callback = {
                onSuccess: function(response) {
                    ...
                    if (vm.submissionResult.count === 0) {
                        vm.showPagination = false;
                        vm.paginationMsg = "نتیجه ای پیدا نشد";
                    } else {
        ...
        vm.toggleLeaderboard = function(getAllEntries){
            ...
            if (vm.phaseSplitId) {
                if (vm.getAllEntries){
                    vm.getAllEntriesTestOption = "پنهان کردن موارد ارسالی خصوصی";
        ...
        vm.createNewTeam = function() {
            ...
            parameters.callback = {
                onSuccess: function() {
                    $rootScope.notify("success", "گروه " + vm.team.name + "  با موفقیت ایجاد شد!");
                    vm.team.error = false;
                    vm.stopLoader();
                    vm.team = {};
                ...
                onError: function(response) {
                    ...
                    vm.stopLoader();
                    $rootScope.notify("error", "گروه جدید ایجاد نشد");
                }
            };
                ...
                onSuccess: function(response) {
                    ...
                    if (status === 200) {
                      ...
                      if (detail['is_public'] == true) {
                        message = "ارسال به اطلاع عموم می رسد.";
                      }
                      else {
                        message = "ارسال خصوصی است.";
                      }
        ...
        vm.downloadChallengeSubmissions = function() {
            if (vm.phaseId) {
                ...
                $rootScope.notify("error", "لطفا مرحله مسابقه را انتخاب کنید!");
            }
        };
        ...
        vm.cancelSubmission = function(submissionId) {
            ...
            parameters.callback = {
                onSuccess: function(response) {
                    ...
                    if (status === 200) {
                        $mdDialog.hide();
                        $rootScope.notify("success", "ارسال با موفقیت لغو شد!");
                    }
                },
                onError: function(response) {
                    $mdDialog.hide();
        ...
        vm.showCancelSubmissionDialog = function(submissionId, status) {
            if (status != "submitted") {
                $rootScope.notify("error", "فقط ارسال های پردازش نشده را می توان لغو کرد");
                return;
            }
            vm.submissionId = submissionId;
            $mdDialog.show({
            ...
            if (updateSubmissionMetaDataForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "داده ها با موفقیت به روزرسانی شد!");
        ...
        vm.verifySubmission = function(submissionId, isVerified) {
            ...
            parameters.callback = {
                onSuccess: function(response) {
                    var status = response.status;
                    if (status === 200) {
                        $rootScope.notify("success", "وضعیت تأیید با موفقیت به روزرسانی شد!");
                    }
                },
                onError: function(response) {
                    var error = response.data;
            ...
            if (editChallengeOverviewForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "توضیحات با موفقیت به روزرسانی شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
            ...
            if (deleteChallengeForm){
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 204){
                            $mdDialog.hide();
                            $rootScope.notify("success", "مسابقه با موفقیت حذف شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
            ...
            if (editSubmissionGuidelinesForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "دستورالعمل ارسال با موفقیت به روزرسانی شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
            ...
            if (editEvaluationCriteriaForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "جزئیات ارزیابی با موفقیت به روزرسانی شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
        ...
        vm.editEvalScript = function(editEvaluationCriteriaForm) {
            if (editEvaluationCriteriaForm) {
                if (vm.editEvaluationScript === undefined || vm.editEvaluationScript === null
                    ...
                    var error = "لطفا یک اسکریپت ارزیابی معتبر بارگذاری کنید!";
                    $mdDialog.hide();
                    $rootScope.notify("error", error);
                    return;
                }
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "اسکریپت ارزیابی با موفقیت به روزسانی شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
            ...
            if (editTermsAndConditionsForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "شرایط و ضوابط با موفقیت به روزرسانی شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
            ...
            if (editChallengeTitleForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "عنوان مسابقه با موفقیت به روزرسانی شد!");
                        }
                    },
                    onError: function(response) {
                        $mdDialog.hide();
            ...
            if (editChallengePhaseForm) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        ...
                        if (status === 200) {
                            $mdDialog.hide();
                            $rootScope.notify("success", "جزئیات مرحله مسابقه با موفقیت به روزرسانی شد!");
                        }
                    },
                    onError: function(response) {
                        utilities.hideLoader();
        ...
        vm.editChallengeDate = function(editChallengeDateForm) {
            if (editChallengeDateForm) {
                ...
                if (new Date(vm.challengeStartDate).valueOf() < new Date(vm.challengeEndDate).valueOf()) {
                    ...
                    parameters.callback = {
                        onSuccess: function(response) {
                            ...
                            if (status === 200) {
                                ...
                                $mdDialog.hide();
                                $rootScope.notify("success", "تاریخ شروع و پایان مسابقه با موفقیت به روزرسانی شد!");
                            }
                        },
                        onError: function(response) {
                            utilities.hideLoader();
                    ...
                    $rootScope.notify("error", "تاریخ شروع مسابقه نمی تواند یکسان یا بزرگتر از تاریخ پایان باشد.");
                }
            } else {
                $mdDialog.hide();
            }
...
(function() {
    ...
    function ChallengeHostTeamsCtrl(utilities, loaderService, $scope, $state, $http, $rootScope, $mdDialog) {
        ...
        vm.updateChallengeHostTeamData = function(updateChallengeHostTeamDataForm) {
            ...
            parameters.callback = {
                onSuccess: function() {
                    $mdDialog.hide();
                    vm.team = {};
                    $rootScope.notify("success", "گروه میزبان به روز شد!");
                    var parameters = {};
        ...
        vm.confirmDelete = function(ev, hostTeamId) {
            ...
            var confirm = $mdDialog.confirm()
                .title('آیا می خواهید خود را حذف کنید؟')
                .textContent('توجه: این عمل شما را از گروه حذف می کند.')
                .ariaLabel('Lucky day')
                .targetEvent(ev)
                .ok('بله')
                .cancel("خیر");
            ...
            $mdDialog.show(confirm).then(function() {
                ...
                parameters.callback = {
                    onSuccess: function() {
                        vm.team.error = false;
                        $rootScope.notify("info", "شما خود را با موفقیت حذف کردید");
                    ...
                    onError: function() {
                        vm.stopLoader();
                        $rootScope.notify("error", "نتوانستم شما را از مسابقه حذف کنم");
                    }
                };
        ...
        vm.inviteOthers = function(ev, hostTeamId) {
            ...
            var confirm = $mdDialog.prompt()
                .title('اعضای دیگر را به گروه خود اضافه کنید')
                .textContent('آدرس ایمیل شخص مورد نظر را وارد کنید')
                .placeholder('email')
                .ariaLabel('')
                .targetEvent(ev)
                .ok('Add')
            ...
            $mdDialog.show(confirm).then(function(result) {
                ...
                parameters.callback = {
                    onSuccess: function() {
                        $rootScope.notify("success", parameters.data.email + " با موفقیت اضافه شد");
                    },
                    onError: function(response) {
                        var error = response.data.error;
                        $rootScope.notify("error", error);
          ...
          if(resetconfirmFormValid){
            ...
            parameters.token = userKey;
                parameters.callback = {
                    onSuccess: function() {
                        vm.user.error = false;
                        $rootScope.notify("success", "گذرواژه ی شما با موفقیت تغییر کرد!");
                        vm.stopLoader();
                        ...
                        if (response.status == 400) {
                            ...
                            try {
                                ...
                                if (isUsernameValid) {
                                    ...
                                    $rootScope.notify("error", "برخی از خطاها رخ داده است. لطفا دوباره تلاش کنید!");
                                }
            ...
            onSuccess: function(response) {
                ...
                if (status == 200) {
                    if (results.length !== 0) {
                        ...
                        vm.noTeamDisplay = "گروه به زودی به روزرسانی میشود!";
                    }
                }
            },
            onError: function() {}
...
(function() {
    ...
    function profileCtrl(utilities, $rootScope, $scope, $mdDialog, moment) {
        ...
        vm.showConfirmation = function(){
            $rootScope.notify("success", "توکن شما کپی شد.");
        };
...
(function() {
    ...
    function TeamsCtrl(utilities,loaderService, $scope, $state, $http, $rootScope, $mdDialog) {
        ...
        vm.inviteOthers = function(ev, participantTeamId) {
            ...
            // Appending dialog to document.body to cover sidenav in docs app
            var confirm = $mdDialog.prompt()
                .title('اعضای دیگر را به این تیم اضافه کنید')
                .textContent('آدرس ایمیل شخص مورد نظر را وارد کنید')
                .placeholder('email')
                .ariaLabel('')
                .targetEvent(ev)
        ...
        vm.updateParticipantTeamData = function(updateParticipantTeamDataForm) {
            ...
            parameters.callback = {
                onSuccess: function() {
                    $mdDialog.hide();
                    vm.team = {};
                    $rootScope.notify("success", "گروه شرکت کننده به روز شد!");
                    var parameters = {};
                    // Retrives the updated lists and displays it.
                    parameters.url = 'participants/participant_team';
                    parameters.method = 'GET';
...
(function() {
    ...
    function updateProfileCtrl(utilities, $state, $rootScope) {
        ...
        parameters.callback = {
            ...
            onError: function() {
                $rootScope.notify("error", "خطا در بارگیری پروفایل, لطفا دوباره تلاش کنید later !");
            }
        };
        ...
        vm.updateProfile = function(resetconfirmFormValid) {
            if (resetconfirmFormValid) {
                ...
                parameters.callback = {
                    onSuccess: function(response) {
                        if (response.status == 200) {
                            $rootScope.notify("success", "پرروفایل با موفقیت به روز شد!");
                ...
                $rootScope.notify("error", "فیلدهای فرم معتبر نیستند!");
                vm.stopLoader();
            }
        };
    }
